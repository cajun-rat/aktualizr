...
TEST(OfflineLogsDb, OpenRefusesSymlinkToFile) {
  ...
  {
    auto real_db = OfflineLogsDb::Open(real_db_path);
    ASSERT_NE(real_db, nullptr);
  }
  ...
  auto db = OfflineLogsDb::Open(symlink_path);
  EXPECT_EQ(db, nullptr) << "Open() should refuse to follow symlinks";
}
...
TEST(OfflineLogsDb, OpenRefusesSymlinkToNonexistent) {
  ...
  auto db = OfflineLogsDb::Open(symlink_path);
  EXPECT_EQ(db, nullptr) << "Open() should refuse to follow symlinks even to non-existent targets";
}
...
TEST(OfflineLogsDb, OpenSucceedsForRegularFile) {
  ...
  auto db = OfflineLogsDb::Open(db_path);
  EXPECT_NE(db, nullptr) << "Open() should succeed for regular files";
}
