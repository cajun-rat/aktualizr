  ...
  for (int version_to_send = sec_root_version + 1; version_to_send < last_root_version; version_to_send++) {
    ...
    try {
      ...
      if (!result.isSuccess()) {
        ...
        // support the Root version request amd skip sending any root metadata.
        ...
        if (version_to_send == 1) {
          LOG_WARNING
              << "Sending root.1.json to a secondary failed. Assuming it doesn't allow root rotation and contining.";
